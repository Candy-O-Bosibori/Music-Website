...
fetch('https://musicapi-1p7w.onrender.com/allSongs')
...
// Load the song and fetch comments
...
// Delete comment
function deleteComment(deleteButton, commentId, musicIndex) {
    const commentElement = deleteButton.parentNode;
    commentsContainer.removeChild(commentElement);

    deleteCommentAPI(commentId, musicIndex);
}

// Function to make a DELETE request and remove a comment from the API
function deleteCommentAPI(commentId, musicIndex) {
    const apiUrl = `https://musicapi-1p7w.onrender.com/allSongs/${musicIndex}/comments/${commentId}`;
...
// Function to fetch comments for a specific song
function fetchComments(index) {
    const apiUrl = `https://musicapi-1p7w.onrender.com/allSongs/${index}/comments`;
...
function displayComments(comments) {
    ...
    comments.forEach(comment => {
        ...
        newComment.innerHTML = `${comment.text} 
            <span class="delete-btn" onclick="deleteComment(this, ${comment.id}, ${musicIndex})">Delete</span>`;
...
commentForm.addEventListener('submit', function (event) {
    ...
    if (comment.trim() !== '') {
        ...
        newComment.innerHTML = `${comment} 
            <span class="delete-btn" onclick="deleteComment(this, null, ${musicIndex})">Delete</span>`;
        commentsContainer.appendChild(newComment);

        // Send the comment to the API
        addCommentAPI(comment, musicIndex);
...
function addCommentAPI(comment, musicIndex) {
    const apiUrl = `https://musicapi-1p7w.onrender.com/allSongs/${musicIndex}/comments`;
